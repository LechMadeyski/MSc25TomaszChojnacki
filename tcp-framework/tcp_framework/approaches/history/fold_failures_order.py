...
class FoldFailuresOrder(TcpApproach):
    def __init__(self, func: Literal["dfe"] | Literal["total"] | Literal["last"] | Literal["recent"]) -> None:
        self._failures: DefaultDict[TestCase, list[int]] = defaultdict(lambda: [])
        match func:
            case "dfe": self._func = _dfe
            case "total": self._func = _total
            case "last": self._func = _last
            case "recent": self._func = _recent
